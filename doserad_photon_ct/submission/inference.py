...
from doserad_photon_ct.conditioning import PhotonCondition, SpatialGeometry
from doserad_photon_ct.inference import predict_conditioned_arrays, warmup_model
...
CT_DIRECTORY_BASE = "radiation-dose-calculation-source-ct-image"
...
def _find_input_image(image_file_idx: int) -> Path:
    directory = INPUT_PATH / "images" / f"{CT_DIRECTORY_BASE}-{image_file_idx + 1}"
...
def run(bundle: ModelBundle) -> None:
    ...
    try:
        for image_entry in metadata:
            ...
            ct_path = _find_input_image(image_file_idx)
            reference = sitk.ReadImage(str(ct_path))
            ct = sitk.GetArrayFromImage(reference).astype(np.float32, copy=False)
            geometry = _geometry_from_sitk(reference)
            print(
                f"Image {image_file_idx}: shape={ct.shape}, "
                f"spacing={reference.GetSpacing()}",
                flush=True,
            )
            ...
            # optional there, so enumeration order is the fallback.
                ...
                for cp_position, control_point in enumerate(beam["control_points"]):
                    ...
                    jobs.append(
                        ControlPointJob(
                            ...
                            stack_index=int(output_info["idx_in_output"]),
                            minimum_cutoff=float(
                                output_info.get("minimum_cutoff", 0.0)
                            ),
            ...
            for offset in range(0, len(jobs), chunk_size):
                ...
                predictions = predict_conditioned_arrays(
                    ...
                    include_density=bundle.include_density,
                    include_physics_priors=bundle.include_physics_priors,
                    include_radiological_depth=bundle.include_radiological_depth,
            ...
            print(
                ...
                f"seconds={image_seconds:.3f}, "
                f"maps/s={len(jobs) / max(image_seconds, 1e-9):.3f}",
                flush=True,
            )
