#!/usr/bin/env bash
# Offline container smoke test for the Photon CT submission image.
#
...
IMAGE="doserad2026-photon-ct:latest"
MODEL_DIR="$PROJECT_DIR/dist"
if [[ ! -f "$MODEL_DIR/best.pt" ]]; then
  echo "Missing portable checkpoint $MODEL_DIR/best.pt" >&2
  exit 1
fi

WORK=$(mktemp -d)
NETWORK="doserad-smoke-$$"
CONTAINER="doserad-smoke-photon-ct-$$"
cleanup() {
...
docker run --detach --name "$CONTAINER" \
  ...
  --env MODEL_CHECKPOINT=best.pt \
  --env CONTROL_POINT_CHUNK_SIZE="${CONTROL_POINT_CHUNK_SIZE:-2}" \
...
echo "PASS: photon-ct offline smoke test, slot 1 stacked $frames frame(s)"
