...
class PatientAudit:
    ...
    complete_for_photon_ct: bool = False
...
def audit_patient(patient_dir: Path, check_headers: bool = False) -> PatientAudit:
    ...
    if not result.ct_exists:
        result.errors.append("CT image is missing")
    ...
    if check_headers and result.ct_exists:
        try:
            ct_header = read_mha_header(ct_path)
            ...
            for dose_name in sorted(existing_names & expected_names):
                ...
                mismatches = [
                    key for key in geometry_keys if ct_header.get(key) != dose_header.get(key)
                ]
                if mismatches:
                    result.errors.append(
                        f"{dose_name}: geometry differs from CT ({', '.join(mismatches)})"
                    )
        except (OSError, UnicodeDecodeError, ValueError) as exc:
            result.errors.append(f"MHA header: {exc}")

    result.complete_for_photon_ct = (
        result.metadata_exists
        and result.ct_exists
...
def audit_dataset(data_root: Path, check_headers: bool = False) -> AuditSummary:
    ...
    complete = sum(patient.complete_for_photon_ct for patient in patients)
    return AuditSummary(
...
def build_manifest(summary: AuditSummary, output_path: Path) -> int:
    """Write one row per beam/control-point for complete photon-CT patients."""
