...
from doserad_photon_ct.inference import predict_conditioned_arrays, warmup_model  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--condition-chunk-size", type=int, default=0)
...
def main() -> int:
    ...
    ct = np.asarray(load_mha_array(records[0].ct_path), dtype=np.float32)
    geometry = SpatialGeometry.from_mha_header(read_mha_header(records[0].ct_path))
    ...
    inference_kwargs = dict(
        ...
        device=device,
        patch_size_zyx=tuple(args.patch_size),
        dose_scale=float(config["dose_scale"]),
    ...
    def predict_all() -> list[np.ndarray]:
        if args.condition_chunk_size <= 0:
            return predict_conditioned_arrays(**inference_kwargs)
        outputs = []
        for offset in range(0, len(conditions), args.condition_chunk_size):
            ...
            outputs.extend(predict_conditioned_arrays(**chunk_kwargs))
        return outputs
