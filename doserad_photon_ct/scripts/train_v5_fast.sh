#!/usr/bin/env bash
# v5: leaderboard-calibrated recipe. base16-L5 = 13.9M params at only 1.25x
# v3 inference cost (level 5 runs on downsampled grids). Runtime budget on the
# official leaderboard is rank-scored with double weight, so bigger models
# (v4-56M, 2.35x cost) are strictly worse operating points.
set -euo pipefail
...
python3 scripts/train.py \
  --device cuda \
  --output-dir runs/photon_ct_v5_fast \
  --epochs 60 \
