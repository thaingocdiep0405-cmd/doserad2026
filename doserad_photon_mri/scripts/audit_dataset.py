...
from doserad_photon_mri import audit_dataset, build_manifest  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit DoseRAD2026 photon-MRI files and build a training manifest."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--check-headers",
        action="store_true",
        help="Compare MRI and dose MHA geometry without decompressing voxel data",
    )
    parser.add_argument(
        "--require-complete",
        action="store_true",
...
def main() -> int:
    ...
    print(f"Complete photon-MRI:  {summary.complete_patient_count}")
    ...
    if summary.incomplete_patient_count:
        ...
        for patient in summary.patients:
            if not patient.complete_for_photon_mri:
                print(
                    f"- {patient.patient_id}: MRI={patient.mr_exists}, "
