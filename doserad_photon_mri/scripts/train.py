...
from doserad_photon_mri.dataset import PhotonMRIPatchDataset, lookup_condition  # noqa: E402
from doserad_photon_mri.dataset_index import read_mha_header  # noqa: E402
from doserad_photon_mri.inference import predict_record_volume  # noqa: E402
from doserad_photon_mri.losses import (  # noqa: E402
    DoseLoss,
    LossConfig,
...
from doserad_photon_mri.metrics import beam_direction, idd_curve_distance  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train DoseRAD2026 photon-MRI patch model.")
    ...
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "runs/photon_mri_baseline")
    parser.add_argument("--resume", type=Path)
    parser.add_argument(
        "--init-checkpoint",
        type=Path,
        help="Warm-start model weights only; Task 1 six-channel weights are compatible",
    )
    ...
    parser.add_argument("--image-cache-size", type=int, default=2)
    ...
    parser.add_argument(
        "--physics-priors",
        action="store_true",
        help="Add aperture-edge, inverse-square, field-width and fluence priors",
    )
    ...
    parser.add_argument("--aug-intensity-jitter", type=float, default=0.05)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Auxiliary pseudo-density loss weight; paired CT is used only during training",
    )
...
def make_loaders(
...
) -> tuple[PhotonMRIPatchDataset, DataLoader, PhotonMRIPatchDataset, DataLoader]:
    common: dict[str, Any] = {
        ...
        "include_density_target": args.density_aux_weight > 0,
    }
    train_dataset = PhotonMRIPatchDataset(
        split="train",
    ...
    validation_dataset = PhotonMRIPatchDataset(
...
def forward_training_outputs(
    ...
    inputs: torch.Tensor,
    use_density_auxiliary: bool,
) -> tuple[torch.Tensor | list[torch.Tensor], torch.Tensor | None]:
    if not use_density_auxiliary:
        return model(inputs), None
    return model.forward_with_auxiliary(inputs)
...
def density_auxiliary_loss(
    ...
    if prediction is None or target is None:
        raise RuntimeError("density auxiliary head and paired CT target are required")
    if mask is not None and mask.any():
        return torch.nn.functional.smooth_l1_loss(
            prediction[mask], target[mask], beta=0.05
        )
    return torch.nn.functional.smooth_l1_loss(prediction, target, beta=0.05)
...
def train_one_epoch(
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        with autocast_context(device, amp_enabled):
            predictions, density_prediction = forward_training_outputs(
                model, inputs, args.density_aux_weight > 0
            )
            ...
            if args.density_aux_weight > 0:
                density_target = batch["density_target"].to(device, non_blocking=True)
                density_mask = batch["density_mask"].to(device, non_blocking=True)
                density_loss = density_auxiliary_loss(
                    density_prediction, density_target, density_mask
                )
                loss = loss + args.density_aux_weight * density_loss
                components["density"] = density_loss
...
def validate_full_volumes(
    model: torch.nn.Module,
    dataset: PhotonMRIPatchDataset,
    device: torch.device,
    args: argparse.Namespace,
) -> dict[str, float]:
    # Patient-balanced validation: cover beams and arc positions instead of
    # evaluating consecutive rows from one patient/beam.
    ...
    for record in records:
        ...
        spacing_xyz = tuple(
            float(value)
            for value in read_mha_header(record.image_path)["ElementSpacing"].split()
        )
...
def main() -> int:
    ...
    if args.density_aux_weight < 0:
        raise ValueError("density-aux-weight must be non-negative")
    ...
    model_config = ModelConfig(
        in_channels=10 if args.physics_priors else 6,
        base_channels=args.base_channels,
        levels=args.levels,
        blocks_per_level=args.blocks_per_level,
        dropout=args.dropout,
        physics_priors=args.physics_priors,
        auxiliary_density=args.density_aux_weight > 0,
    ...
    if args.init_checkpoint:
        ...
        allowed_missing = set()
        if args.density_aux_weight > 0:
            allowed_missing |= {"density_output.weight", "density_output.bias"}
        if args.deep_supervision:
            allowed_missing |= {
                k for k in model.state_dict() if k.startswith("ds_heads.")
            }
