    ...
    for mode in args.modes:
        ...
        for patient_id, patient_records in sorted(by_patient.items()):
            ...
            predictions = predict_conditioned_arrays(
                ...
                wepl_cache={},
                # The submission keeps one window cache per image, so the
                # benchmark has to as well: without it every window rebuilds
                # and re-uploads its static channels and the measured cost is
                # not the cost the graded container pays.
                window_cache={},
            )
            if args.prediction_scale != 1.0:
                predictions = [
                    np.asarray(prediction * np.float32(args.prediction_scale), dtype=np.float32)
